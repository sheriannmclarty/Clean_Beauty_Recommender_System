...
st.set_page_config(
    page_title="Skintone Beauty Recommender",
    page_icon="🌿",
    layout="wide"
)
...
<style>
    ...
    [data-testid="stSidebar"] {
        background-color: #6A173D !important;
    }
    ...
    .stButton > button:hover {
        background-color: #934261 !important;
        color: white !important;
    }
    ...
    .rec-card {
        ...
        border-radius: 16px;
        border-left: 7px solid #6A173D;
        margin-bottom: 1.1rem;
        box-shadow: 0 4px 14px rgba(92,29,58,0.14);
    }
    ...
    .rec-card-caution {
        ...
        padding: 1.25rem 1.4rem;
        border-radius: 16px;
        border-left: 7px solid #C36A22;
        margin-bottom: 1.1rem;
        box-shadow: 0 4px 14px rgba(195,106,34,0.14);
    }
...
def explain_tone_signal(tone_score, category=""):
    ...
    if tone_score > 0.1:
        return {
            ...
            "emoji": "✅",
            "explanation": (
                "Reviews include positive tone-related language such as good match, no cast, "
                "works well on deeper skin, or better complexion performance."
            ),
        }
    ...
    if tone_score < -0.1:
        return {
            ...
            "emoji": "❌",
            "explanation": (
                "Reviews include concern terms such as white cast, ashiness, oxidation, "
                "flashback, too orange, gray finish, or undertone mismatch."
            ),
        }
    ...
    return {
        ...
        "emoji": "⚠️",
        "explanation": (
            "There was not enough complexion-specific review language to confirm shade or undertone performance."
            if is_complexion
...
def skin_type_match_score(rec, skin_type):
    ...
    product_text = " ".join(
        [
            ...
            str(rec.get("primary_category", "")),
            str(rec.get("secondary_category", "")),
            str(rec.get("tertiary_category", "")),
        ]
    ).lower()

    skin_type_keywords = {
        "Dry": [
            "hydrating", "hydration", "moisture", "moisturizing", "cream",
            "barrier", "ceramide", "squalane", "hyaluronic", "balm"
        ],
        "Oily": [
            "oil-free", "oil free", "gel", "lightweight", "matte",
            "pore", "clarifying", "non-comedogenic", "shine", "sebum"
        ],
        "Combination": [
            "balancing", "lightweight", "hydrating", "gel cream",
            "barrier", "pore", "daily"
        ],
        "Normal": [
            "daily", "hydrating", "moisturizer", "gentle", "cream"
        ],
        "Sensitive": [
            "sensitive", "gentle", "calming", "soothing", "fragrance-free",
            "fragrance free", "barrier", "ceramide", "cica", "colloidal oatmeal"
        ],
    }
...
def filter_by_category(results, category):
    ...
    for rec in results:
        combined = (
            f"{rec.get('primary_category', '')} "
            f"{rec.get('secondary_category', '')} "
            f"{rec.get('tertiary_category', '')} "
            f"{rec.get('product_name', '')}"
        ).lower()
...
def get_users_for_tone(df, tone_labels, min_reviews=2):
    ...
    users = user_counts[user_counts >= min_reviews].index.tolist()
...
ethnic_first_names = [
    "Aaliyah", "Imani", "Zuri", "Keisha", "Latoya", "Ebony", "Destiny",
    "Jasmine", "Tamara", "Shayla", "Monique", "Tiana", "Amara", "Chioma",
    "Fatima", "Adaeze", "Ngozi", "Nia", "Zainab", "Aisha", "Blessing",
    "Chiamaka", "Folake", "Yetunde", "Gabriela", "Valentina", "Marisol",
    "Xiomara", "Catalina", "Lucia", "Priya", "Ananya", "Divya", "Meena",
    "Kavya", "Shreya", "Nisha", "Sheriann", "Sharai", "Abena", "Solange",
    "Kaydean", "Marsha", "Andrea", "Charmaine", "Juliet", "Melissa",
    "Jaleesa", "Simone", "Jessica", "Angie", "Nadine", "Camille", "Donna",
    "Sabrina", "Stacyann", "Rochelle", "Tanisha", "Khadijah", "Naomi",
    "Temi", "Tosin", "Esi", "Akosua", "Sade", "Kemi", "Funmi", "Yewande",
    "Nkechi"
]

ethnic_last_names = [
    "Washington", "Jackson", "Williams", "Thompson", "Robinson", "Okafor",
    "Adeyemi", "Nwosu", "Mensah", "Diallo", "Traore", "Rodriguez",
    "Martinez", "Flores", "Rivera", "Reyes", "Patel", "Sharma",
    "Krishnan", "Iyer", "Nair", "Smith", "McLarty", "Jean-Pierre",
    "Green", "Lopez", "Johnson", "West", "McIntosh", "Roper", "Gonzalez",
    "Bailey", "James", "Campbell", "Hinds", "Montgomery", "Henry",
    "Edwards", "King", "Clarke", "Reid", "Grant", "Francis", "Bennett",
    "Gordon", "Boateng", "Adebayo", "Okonkwo", "Eze", "Owusu", "Santiago",
    "Torres", "Morales", "Cruz", "Persaud", "Singh"
]
...
nut_allergens = [
    ...
    "macadamia oil", "walnut oil", "hazelnut oil", "cashew", "brazil nut",
    "pecan", "pistachio", "coconut oil", "coconut acid", "coconut water",
    "cocos nucifera", "butyrospermum parkii", "argania spinosa",
    "prunus amygdalus", "carthamus tinctorius"
]

sensitivity_terms = [
    "eczema", "rash", "breakout", "irritat", "reaction",
    "burn", "sting", "flare", "allergic", "hives"
]

category_keywords = {
    "Moisturizers": [
        "moisturizer", "moisturizers", "cream", "face cream",
        "hydrating cream", "daily moisturizer"
    ],
    "Serums & Treatments": [
        "treatment", "treatments", "serum", "retinol", "acid",
        "peel", "exfoliant", "brightening", "dark spot", "acne"
    ],
    "Cleansers": [
        "cleanser", "cleansing", "wash", "gel cleanser", "foam",
        "balm cleanser", "oil cleanser"
    ],
    "Sunscreen": [
        "sunscreen", "spf", "sun screen", "uv", "sun protection",
        "mineral sunscreen"
    ],
    "Foundation & Complexion": [
        "foundation", "concealer", "skin tint", "tinted moisturizer",
        "bb cream", "cc cream", "complexion", "face makeup",
        "setting powder", "powder foundation", "tint", "mineral powder"
    ],
}
...
skin_tone_options = {
    "Any Melanin-Rich Tone": {
        "labels": ["tan", "deeptan", "deep tan", "dark", "deep", "rich", "ebony"],
        "hex": "#8A5536",
        "description": "Uses any available melanin-rich user profile.",
    },
    "Tan / Deep Tan": {
        "labels": ["tan", "deeptan", "deep tan"],
        "hex": "#B8794D",
        "description": "Warm tan to deep tan complexion range.",
    },
    "Dark": {
        "labels": ["dark"],
        "hex": "#8A5536",
        "description": "Dark brown complexion range.",
    },
    "Deep": {
        "labels": ["deep"],
        "hex": "#6A3F2A",
        "description": "Deep brown complexion range.",
    },
    "Rich": {
        "labels": ["rich"],
        "hex": "#4A2A1C",
        "description": "Rich deep complexion range.",
    },
    "Ebony": {
        "labels": ["ebony"],
        "hex": "#2B1711",
        "description": "Ebony / deepest complexion range.",
    },
}

shade_profiles = {
    "Tan / Deep Tan": ["tan", "deeptan", "deep tan"],
    "Dark": ["dark"],
    "Deep": ["deep"],
    "Rich": ["rich"],
    "Ebony": ["ebony"],
...
@st.cache_data
def load_products():
    products_df = read_csv_safely(DATA_DIR / "product_info.csv")
...
def load_cosing():
    ...
    annex3 = read_csv_safely(DATA_DIR / "cosing_annex3_restricted_v2.txt", skiprows=4)

    annex2["ingredient_clean"] = (
        annex2["Chemical name / INN"]
        .astype(str)
        .str.strip()
        .str.lower()
    )

    annex3["ingredient_clean"] = (
        annex3["Chemical name / INN"]
        .astype(str)
        .str.strip()
        .str.lower()
    )
...
def compute_product_sensitivity(df):
    return (
        df.groupby("product_name_x")["review_text"]
        .apply(
            lambda reviews: sum(
                any(term in str(review).lower() for term in sensitivity_terms)
...
class BiasAdjustedRecommender:
    ...
    def predict(self, user, item):
        raw = (
            self.global_avg
            + self.user_biases.get(user, 0)
            + self.item_biases.get(item, 0)
        )
    ...
    def recommend(self, user_id, top_n=200):
        ...
        return [
            {"item": int(item), "predicted_rating": round(score, 2)}
...
def build_model():
    ...
    item_to_meta = (
        ...
        .set_index("item")[["product_name_x", "brand_name_x"]]
...
selected_tone_group = st.sidebar.radio(
    "Select a skin tone range:",
    list(skin_tone_options.keys()),
    index=0,
)
...
<div style="background:rgba(255,255,255,0.10);padding:14px;border-radius:16px;border:1px solid rgba(255,255,255,0.35);margin-top:10px;margin-bottom:12px;">
    ...
    <div style="text-align:center;color:white;">
        <strong>{selected_tone_group}</strong><br>
        <span style="font-size:0.88rem;">{selected_tone_info["description"]}</span>
    </div>
</div>
""",
    unsafe_allow_html=True,
...
sensitive_skin = st.sidebar.radio(
    "Do you have sensitive skin or eczema?",
    ["No", "Yes — filter irritants"],
)
...
st.markdown(
    "Choose your product category, skin tone profile, skin type, and ingredient concerns. "
...
if st.button("✨ Get My Recommendations"):
    ...
    tone_filtered_users = df[
        df["skin_tone"]
        .fillna("")
        .astype(str)
        .str.lower()
        .str.strip()
        .isin(selected_skin_tones)
    ]
    ...
    if not top_users:
        st.warning(
            f"Not enough review profiles found for {selected_tone_group}. "
            "Showing Any Melanin-Rich Tone instead."
        )
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:1rem 0 1.5rem 0;">
    <div style="background:#FFF8F3;padding:14px;border-radius:14px;">
        <div style="color:#6A173D;font-weight:700;">👤 Demo Reviewer Profile</div>
        <div style="font-size:1.2rem;color:#2B1B24;">{display_name}</div>
    </div>
    <div style="background:#FFF8F3;padding:14px;border-radius:14px;">
        <div style="color:#6A173D;font-weight:700;">🎨 Skin Tone</div>
        <div style="font-size:1.35rem;color:#2B1B24;">{str(user_row["skin_tone"]).capitalize()}</div>
    </div>
    <div style="background:#FFF8F3;padding:14px;border-radius:14px;">
        <div style="color:#6A173D;font-weight:700;">💧 Skin Type</div>
        <div style="font-size:1.05rem;color:#2B1B24;">{skin_type_display}</div>
    </div>
    <div style="background:#FFF8F3;padding:14px;border-radius:14px;">
        <div style="color:#6A173D;font-weight:700;">📝 Reviews</div>
        <div style="font-size:1.35rem;color:#2B1B24;">{user_counts[top_user]}</div>
    </div>
</div>
""",
        unsafe_allow_html=True,
    ...
    st.caption(
        "Demo names are randomly generated to protect reviewer identity. "
        "Recommendations are based on anonymized review behavior from the selected skin tone profile."
    )
    ...
    with st.spinner("Finding your perfect products..."):
        ...
        for rec in raw_recs:
            ...
            ingredients = clean_ingredients_text(
                product_match.iloc[0]["ingredients"]
                if not product_match.empty
                else None
            )

            primary_cat = (
                str(product_match.iloc[0].get("primary_category", ""))
                if not product_match.empty
                else ""
            )

            secondary_cat = (
                str(product_match.iloc[0].get("secondary_category", ""))
                if not product_match.empty
                else ""
            )

            tertiary_cat = (
                str(product_match.iloc[0].get("tertiary_category", ""))
                if not product_match.empty
                else ""
            )
        ...
        if skin_type != "Not sure / No preference":
            results = sorted(
                results,
                key=lambda x: (
                    x.get("skin_type_score", 0),
                    x.get("tone_score", 0),
                    x.get("predicted_rating", 0),
                ),
                reverse=True,
            )
        ...
        if nut_allergy == "Yes":
            ...
            for rec in results:
                found_allergen = next(
                    (
                        allergen
                        for allergen in nut_allergens
                        if allergen in rec.get("ingredients", "").lower()
                    ),
                    None,
                )
    ...
    if flagged_count > 0:
        st.warning(
            f"⚠️ {flagged_count} product(s) removed before ranking due to nut allergen detection."
        )

    if sensitive_skin == "Yes — filter irritants":
        if irritant_removed > 0:
            st.warning(
                f"🔬 {irritant_removed} product(s) removed before ranking due to sensitive skin irritant screening."
            )
        else:
            st.success(
                "✅ No sensitive-skin irritants were detected in the displayed recommendation pool."
            )

    st.info(
        "A high predicted rating does not automatically mean a product is the best skin tone match. "
    ...
    if results:
        best_matches = [
            ...
            if float(rec.get("tone_score", 0) or 0) >= -0.1
        ]
        ...
        st.caption(
            "Products that passed ingredient screening and have neutral or positive skin tone review signals."
        )
        ...
        if best_matches:
            for i, rec in enumerate(best_matches[:10], 1):
                ...
                ingredients_preview = (
                    ingredients_text[:180] + "..."
                    if len(ingredients_text) > 180
                ...
                if skin_type != "Not sure / No preference":
                    score = rec.get("skin_type_score", 0)
                    skin_label = (
                        f"Matches {skin_type} skin keywords"
                        if score > 0
                        else f"No strong {skin_type.lower()} skin type signal found"
                    )
                    skin_type_line = f"<p><strong>💧 Skin Type Match:</strong> {skin_label}</p>"
...
"""
        ...
        if use_caution:
            ...
            st.caption(
                "These products passed ingredient safety screening but may have negative skin tone compatibility signals in review text."
            )
...
"""
        ...
        st.warning(
            "No foundation or complexion products were found in the current recommendation pool. "
            "The active dataset is skincare-focused. Foundation tone-aware ranking is scoped as a Phase 3 extension "
            "requiring a dedicated complexion makeup review dataset."
        )
    else:
    ...
    st.caption(
        "One sample recommendation per category across all shade profiles, showing Skintone supports multiple complexion ranges."
    )
